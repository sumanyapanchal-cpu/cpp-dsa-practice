  ...
  /*LOGIC-
      ...
      */
      ...
      int DecToBinary(int decNum){
        int ans = 0;
        int pow = 1; // indicating ones place

        while(decNum > 0){
          int rem = decNum % 2; //remainder dedega i.e last digit
          decNum = decNum/2; //actual quotient dega

          ans += (rem*pow);
          pow *= 10; // moving to tens place in the 1st iteration then hundreds place in the 2nd iteration and so on..
        }
        return ans;
      }
